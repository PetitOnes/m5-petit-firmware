...
#### 方法A: WiFi経由で自動送信（推奨・SDカードをPCに挿す必要なし）

空のSDカード（FAT32）をM5に挿したまま、手順2のセットアップ完了後にPCから:
...
リポジトリ同梱の標準アセット一式（顔7種・効果音12種）がWiFi経由でM5のSDカードに書き込まれます。自作アセットを送る場合は `--assets ディレクトリ`（face/・wav/を含む）を指定。
...
|---|---|---|---|
| 1 | WiFi 1 | DHCP | GL.iNET MT3000等の外出用ルーター（家だけなら自宅WiFi） |
| 2 | WiFi 2（任意） | DHCP | 自宅WiFi |

すべて失敗した場合は 1 秒ごとに WiFi 1→2 の順で再試行し続ける。IPアドレスは常に**DHCPで動的取得**（固定IP設定は廃止）。ルーター側でMACアドレスを見てDHCP予約（常に同じIPを配る設定）をしておくと、Tailscaleのサブネットルートなど固定IP前提の構成と組み合わせやすい。
