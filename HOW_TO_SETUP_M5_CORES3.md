...
4. 設定ページで以下を入力：
   - WiFi 1（必須）・WiFi 2・WiFi 3（任意、この順にフォールバック接続）のSSID・パスワード
...
|---|---|---|---|
| 1 | WiFi 1 | DHCP | 自宅WiFi |
| 2 | WiFi 2 | DHCP | GL.iNET MT3000等の旅行用ルーター |
| 3 | WiFi 3（任意） | DHCP | 予備 |

すべて失敗した場合は 1 秒ごとに WiFi 1→2→3 の順で再試行し続ける。IPアドレスは常に**DHCPで動的取得**（固定IP設定は廃止）。ルーター側でMACアドレスを見てDHCP予約（常に同じIPを配る設定）をしておくと、Tailscaleのサブネットルートなど固定IP前提の構成と組み合わせやすい。
